...
public:
    ...
    PointLight(float posX, float posY, float posZ, float ambR, float ambG, float ambB, float diffR, float diffG, float diffB, float specR, float specG, float specB, float emiR, float emiG, float emiB, float constant, float linear, float quadratic, int num) {

        position = glm::vec3(posX, posY, posZ);
        ambient = glm::vec3(ambR, ambG, ambB);
        diffuse = glm::vec3(diffR, diffG, diffB);
        specular = glm::vec3(specR, specG, specB);
        emission = glm::vec3(emiR, emiG, emiB);
        k_c = constant;
        k_l = linear;
        k_q = quadratic;
        lightNumber = num;
    ...
    void setUpPointLight(Shader& lightingShader)
    {
        ...
        if (lightNumber == 1) {
            ...
            lightingShader.setVec3("pointLights[0].emission", emissionOn * emission);
